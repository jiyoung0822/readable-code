...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            int seletedColIndex = convertColFrom(cellInputCol);
            int selectedRowIndex = convertRowFrom(cellInputRow);
            if (userActionInput.equals("2")) {
                board[selectedRowIndex][seletedColIndex] = "⚑";
                checkIfGameIsOver();
            } else if (userActionInput.equals("1")) {
                if (landMines[selectedRowIndex][seletedColIndex]) {
                    board[selectedRowIndex][seletedColIndex] = "☼";
                    gameStatus = -1;
                    continue;
                } else {
                    open(selectedRowIndex, seletedColIndex);
                }
    ...
    private static void checkIfGameIsOver() {
        boolean isAllOpened = isCellAllOpened();
        if (isAllOpened) {
            gameStatus = 1;
        }
    }

    private static boolean isCellAllOpened() {
        boolean isAllOpened = true;
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 10; col++) {
                if (board[row][col].equals("□")) {
